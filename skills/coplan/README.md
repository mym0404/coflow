...
| `co` CLI | Owns bundle state, validates schema, records events, computes ambiguity, runs Codex CLI agents, and enforces gates. |
...
All other bundle files are CLI-owned.
...
1. Initialize the bundle.
   ...
   `co` creates `exec.yaml`, bundle files, open interview tracks, `status.yaml.review: not_run`, and the first event.
...
5. Score ambiguity with Codex CLI.
   ...
   ```bash
   co planner interview score --runner codex --mode auto
   ```
...
8. Generate skeleton files.
   ...
   `co` writes starter `draft.md`, `plan.yaml`, and `tasks.yaml`.

9. Root agent patches the bundle contract.

   Patch only `draft.md`, `plan.yaml`, and `tasks.yaml` with the smallest correct diff.
...
sequenceDiagram
  ...
  EX->>Bundle: create bundle and event
  Root->>Bundle: read repo separately, not bundle ledger edits
  Root->>EX: interview ask-next --runner codex
  ...
  EX->>Bundle: append round and event
  Root->>EX: interview score --runner codex
  EX->>Codex: scorer prompt
  Codex-->>EX: clarity JSON
  EX->>Bundle: ambiguity.latest and event
