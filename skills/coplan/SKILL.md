...
| `ask_user` | `root_action.question`을 그대로 사용자에게 묻고 답변을 `co.py flow respond --stdin`으로 전달한다. |
| `present_draft` | `root_action.draft`를 그대로 사용자에게 보여주고 approval 또는 feedback을 `co.py flow respond --stdin`으로 전달한다. |
...
| Root agent | `co.py flow`를 실행하고, 질문과 draft를 사용자에게 전달하고, 사용자 응답을 CLI로 되돌려 보낸다. |
...
~/.codex/skills/coplan/scripts/co.py show --file draft|plan|tasks|plan-seed|status|notes|evidence
~/.codex/skills/coplan/scripts/co.py doctor
```

## 금지 사항

- `request.yaml`, `plan_seed.yaml`, `draft.md`, `plan.yaml`, `tasks.yaml`, `planning_context.yaml`, `interview.yaml`, `status.yaml`, `notes.yaml`, `evidence.yaml`을 직접 수정하지 않는다.
